...
# 
    ...
    def anchorPosList(self, ref):   #populates global field: self.sortednodes but also returns it
    ...
    def boundingAnchors(self, strain, startPos, endPos, refstrain): #
        ...
        if len(self.sortednodes) == 0:
            self.sortednodes = self.anchorPosList(refstrain)           
...
# given node name, return dictionary for node coordinates: for floating node, return the coordinates for the base right before the floating node (1 base coordinates)
    ...
    def entity_to_linearcoord(self, entity, offset, strain, k = 11):
        ...
        if entity.startswith('A'):
            ...
            if node.startswith('A'):
                return node_coord[strain] + k + offset
            else:
                return node_coord[strain] + offset
            
    ## given linear coordinates in strain, return graph entity(anchor or edge) plus offset 
    def linear_to_entityoffset(self, coord, strain, refstrain):
        ...
        """
        sanchor, eanchor = self.boundingAnchors(strain, coord, coord, refstrain)
        if sanchor == 'SOURCE':
            offset = coord
        else:
    ...
    def entity_to_anchor(self, entity, offset, ref_node = 'proximal', init_strain = "ABCDEFGHI", k = 11):
        """Given Graph entity (anchor or edge), offset and strain attributes, return linear coordinates for the base position
        ...
        Parameters:
            ...
            init_strain: <str or list> - initial strainlist included in the pangenome
            k: <int> - anchor length
        """
        Strain_Offset = {}
        if entity.startswith('A'):
            if ref_node == "proximal": # default proximal anchor
                for s in init_strain:
            ...
            node = self.edges[entity]['src']
            node_coord = self.Nodecoordinates(node)
            strainlist = set(self.edges[entity]['strain']) & set(init_strain)
            if ref_node == "proximal": # default
                if node.startswith('A'):
                    for s in strainlist:
                        Strain_Offset[s] = node_coord[s] + k + offset
                ...
                if node.startswith('A'):
                    for s in strainlist:
                        ...
                        Strain_Offset[s] = node_coord[s] + k + offset - ref_pos
                else:
                    for s in strainlist:
                        ref_pos = int(self.nodes[ref_node][s])
    ...
    def linear_to_anchoroffset(self, coord, strain, refstrain, ref_node = False ):
        """Given linear coordinates, return graph entity + offset
        Parameters:
            coord: <int> - linear coordinate 
            strain: <str> - strain attributes for the linear coords 
    ...
    def Vertical_linearpos_exchange(self, coord, alter_strain, strain = 'B'):
        ...
        Parameters:
            ...
            strain: <default> "B" path, when multi-alignment cigar are added, this could be further implemented
        """
        entity, offset = self.linear_to_entityoffset(coord, strain)
